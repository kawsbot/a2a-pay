...
export async function runClient(): Promise<{ provider: PublicKey; tx: string }> {
  ...
  for (const s of services) {
    console.log(`  - Owner: ${s.owner.toBase58()}, Price: ${s.price.toString()} lamports`);
  }
  ...
  const tx = await client.createEscrow(cheapest.owner, SERVICE_TYPE, cheapest.price);
  console.log(`[Client] Escrow created! TX: ${tx}`);

  return { provider: cheapest.owner, tx };
}

export async function releaseAfterDelivery(provider: PublicKey): Promise<string> {
  const keypair = loadKeypair();
  const wallet = walletFromKeypair(keypair);
  const client = connectToDevnet(wallet);

  console.log(`[Client] Checking escrow status...`);
  const escrow = await client.getEscrow(client.wallet, provider, SERVICE_TYPE);
  ...
  if ("delivered" in escrow.status) {
    ...
    const tx = await client.releasePayment(provider, SERVICE_TYPE);
